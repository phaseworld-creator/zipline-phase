      ...
      >
        ...
        <Group gap='xs' mb={4} align='center'>
          ...
          <Group gap={6} align='baseline'>
            ...
            <Text style={{ color: '#949ba4', fontSize: 12 }}>{timeStr}</Text>
          </Group>
        </Group>
        ...
        <Box style={{ paddingLeft: 44 }}>
          ...
          >
            {siteName && (
              <Text style={{ color: '#dbdee1', fontSize: 12, marginBottom: 4 }}>
                {resolveVar(siteName)}
              </Text>
            )}
            {title && (
              <Text
                style={{ color: '#00aff4', fontSize: 14, fontWeight: 600, marginBottom: 2, cursor: 'pointer' }}
              >
                {resolveVar(title)}
              </Text>
            )}
            {description && (
              <Text
                style={{ color: '#dbdee1', fontSize: 14, marginTop: 4, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}
              >
                {resolveVar(description)}
              </Text>
            )}
            <Text style={{ color: '#949ba4', fontSize: 11, marginTop: 8 }}>{timeStr}</Text>
          </Box>
        </Box>
      </Box>
...
function Form({ user, setUser }: { user: User; setUser: (u: User) => void }) {
  ...
  const onSubmit = async (values: typeof form.values) => {
    ...
    const { data, error } = await fetchApi<Response['/api/user']>('/api/user', 'PATCH', { view });
    ...
    <Paper withBorder p='sm'>
      ...
      <Stack gap='sm' mt='xs'>
        <form onSubmit={form.onSubmit(onSubmit)}>
          ...
          {/* Embed fields + live Discord preview */}
